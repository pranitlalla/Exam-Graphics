...
layout = html.Div(style={'background-color':'#1D2B53'},children=[
    ...
    html.Div(style={'margin': '0 auto', 'width': '1000px'}, children=[
        dcc.Graph(id='most-scoring-subject_sem_4')
    ]),
    html.Div(style={'margin': '0 auto', 'width': '1000px'}, children=[
        dcc.Graph(id='unsuccessful-scores_sem_4')
    ]),
    html.Div(style={'margin': '0 auto', 'width': '1000px'}, children=[
        dcc.Graph(id='status-pie-chart_sem_4')
    ]),
    html.Div(style={'margin': '0 auto', 'width': '1000px'}, children=[
        dcc.Graph(id='box-plot_sem_4')
    ]),
    html.Div(style={'margin': '0 auto', 'width': '1000px'}, children=[
        dcc.Graph(id='heatmap_sem_4')
    ])
])

@callback(
    Output('table_sem_4', 'columns'),
    Output('table_sem_4', 'data'),
    [Input('class-dropdown_sem_4', 'value'),
     Input('semester-dropdown_sem_4', 'value'),
     Input('subject-dropdown_sem_4', 'value'),
     Input('status-dropdown_sem_4', 'value')],
     allow_duplicate=True
)
def update_table_sem_4(class_values, semester_value, subject_values, status_values):
...
@callback(
    Output('most-scoring-subject_sem_4', 'figure'),
    [Input('table_sem_4', 'data'),
     Input('subject-dropdown_sem_4', 'value')],
     allow_duplicate=True
)
def update_most_scoring_subject_sem_4(rows, subject_values):
...
@callback(
    Output('unsuccessful-scores_sem_4', 'figure'),
    [Input('table_sem_4', 'data')],
    allow_duplicate=True
)
def update_unsuccessful_scores_sem_4(rows):
    df = pd.DataFrame.from_dict(rows)
    subjects = []
...
@callback(
    Output('status-pie-chart_sem_4', 'figure'),
    [Input('table_sem_4', 'data')],
    allow_duplicate=True
)
def update_status_pie_chart_sem_4(rows):
    df = pd.DataFrame.from_dict(rows)
...
@callback(
    Output('box-plot_sem_4', 'figure'),
    [Input('table_sem_4', 'data'),
     Input('subject-dropdown_sem_4', 'value')],
     allow_duplicate=True
)
def update_box_plot_sem_4(rows, subject_values):
    df = pd.DataFrame.from_dict(rows)
...
@callback(
    Output('heatmap_sem_4', 'figure'),
    [Input('table_sem_4', 'data'),
     Input('subject-dropdown_sem_4', 'value')],
     allow_duplicate=True
)
def update_heatmap_sem_4(rows, subject_values):
    df = pd.DataFrame.from_dict(rows)
